...
public class EncryptedData {
  ...
  private String encryptionAlgorithm = "AES/CBC/PKCS7Padding";
   ...
   */
  public EncryptedData(String id, 
          String type, 
          String mimeType,
          String xmlns, 
  ...
  private void initKey()
    ...
    try {
      SecureRandom random = SecureRandom.getInstance(this.secureRandomAlgorithm);
      KeyGenerator keygen = KeyGenerator.getInstance(this.encryptKeyAlg, this.securityProviderName);
  ...
  public Cipher getCipher(int mode, SecretKey transportKey, byte[] iv)
    ...
    try {
      cip = Cipher.getInstance(this.encryptionAlgorithm, this.securityProviderName);
  ...
  public void decrypt(int nKey, int token, String pin)
    ...
    try {
      ...
      m_transportKey = (SecretKey)new SecretKeySpec(decdata, this.encryptionAlgorithm);
      // decrypt data
