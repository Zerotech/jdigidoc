...
import ee.sk.digidoc.services.PKCS12SignatureServiceImpl;
import ee.sk.digidoc.services.SignatureService;
import ee.sk.digidoc.services.SunPKCS11SignatureServiceImpl;
...
public class EncryptedData implements Serializable {
    ...
    private List<EncryptedKey> m_arrEncryptedKeys;
     ...
     */
    public EncryptedData(String id, String type, String mimeType, String xmlns, String encryptionMethod,
                    SignatureService signatureService) throws DigiDocException {
        setId(id);
        setType(type);
    ...
    /**
     * Constructor for EncryptedData without parameters This is to be used only in SAX parser because it initializes
     * instance
     ...
     * @throws DigiDocException for validation errors
     */
    public EncryptedData(String xmlns, SignatureService signatureService) throws DigiDocException {
        m_id = null;
        m_type = null;
        m_mimeType = null;
        setXmlns(xmlns);
    ...
    /**
     * Creates a new <EncryptionProperty> or uses and existing one to store the library namd and version used to create
     * this encrypted
     * document
     * 
    ...
    /**
     * Creates a new <EncryptionProperty> or uses and existing one to store the encrypted document format name and
     * version
     * 
     * @throws DigiDocException
     */
    ...
    public void addEncryptedKey(EncryptedKey key) {
        if (m_arrEncryptedKeys == null) {
            m_arrEncryptedKeys = new ArrayList<EncryptedKey>();
        }
    ...
    private void initKey() throws DigiDocException {
        ...
        if (m_transportKey != null) {
            throw new DigiDocException(DigiDocException.ERR_XMLENC_KEY_STATUS, "Transport key allready initialized!",
                            null);
        }
        try {
        ...
        // check the result
        if (m_transportKey == null) {
            throw new DigiDocException(DigiDocException.ERR_XMLENC_KEY_GEN, "Failure to initialize the transport key!",
                            null);
        }
    }
    ...
    public void encrypt(int nCompressOption) throws DigiDocException {
        ...
        try {
            ...
            if (m_logger.isDebugEnabled()) {
                m_logger.debug("Encrypt - body: " + m_data.length + " full-data: " + (m_data.length - nLastBlockSize)
                                + " left: " + nLastBlockSize);
            }
            ...
            if (m_logger.isDebugEnabled()) {
                m_logger.debug("Encrypt - orig: " + nOrigLen + " input: " + nInput + " encrypted: " + cdata.length);
            }
            // encrypted data
            ...
            if (m_logger.isInfoEnabled()) {
                m_logger.info("Encrypt total - input: " + nTotalInput + " compressed: " + nTotalCompressed
                                + " encrypted: " + nTotalEncrypted);
            }
        } catch (Exception ex) {
            DigiDocException.handleException(ex, DigiDocException.ERR_XMLENC_ENCRYPT);
        }
    ...
    public int getRecvIndex(X509Certificate cert) {
        for (int i = 0; (m_arrEncryptedKeys != null) && (i < m_arrEncryptedKeys.size()); i++) {
            EncryptedKey key = m_arrEncryptedKeys.get(i);
            if (key.getRecipientsCertificate() != null
                            && key.getRecipientsCertificate().getSerialNumber().equals(cert.getSerialNumber())) {
                return i;
            }
        }
        return -1;
    ...
    public void decrypt(int nKey, int token, String pin) throws DigiDocException {
        ...
        try {
            ...
            if (m_logger.isDebugEnabled()) m_logger.debug("Decrypting key: " + nKey + " with token: " + token);
            byte[] decdata = signatureService.decrypt(ekey.getTransportKeyData(), token, pin);
    ...
    public void decryptPkcs12(int nKey, String keystore, String storepass, String storetype) throws DigiDocException {
        ...
        try {
            // decrypt transport key
            ((PKCS12SignatureServiceImpl) signatureService).load(keystore, storetype, storepass);
            if (m_logger.isDebugEnabled()) m_logger.debug("Decrypting key: " + nKey + " with token: " + nKey);
            byte[] decdata = ((PKCS12SignatureServiceImpl) signatureService).decrypt(ekey.getTransportKeyData(), 0,
                            storepass);
            decryptWithKey(decdata);
        } catch (Exception ex) {
            DigiDocException.handleException(ex, DigiDocException.ERR_XMLENC_DECRYPT);
    ...
    public void decrypt(String driver, String keystoreFile, int nKey, int token, String pin) throws DigiDocException {
        ...
        try {
            ...
            if (signatureService == null) {
                ...
                throw new DigiDocException(DigiDocException.ERR_XMLENC_DECRYPT, "No signature factory of type: "
                                + driver, null);
            } else {
                if (signatureService.getType().equals(SignatureService.SIGFAC_TYPE_PKCS12)) {
                    PKCS12SignatureServiceImpl p12sfac = (PKCS12SignatureServiceImpl) signatureService;
                    bOk = p12sfac.load(keystoreFile, SignatureService.SIGFAC_TYPE_PKCS12, pin);
                }
                if (signatureService.getType().equals(SignatureService.SIGFAC_TYPE_PKCS11_SUN)) {
                    SunPKCS11SignatureServiceImpl sFac = (SunPKCS11SignatureServiceImpl) signatureService;
                    bOk = sFac.load(pin, token);
                }
            }
            if (!bOk) {
            ...
            if (m_logger.isDebugEnabled())
                m_logger.debug("Decrypting key: " + nKey + " with token: " + token + " key-data: "
                                + ((ekey.getTransportKeyData() != null) ? ekey.getTransportKeyData().length : 0));
            byte[] decdata = signatureService.decrypt(ekey.getTransportKeyData(), token, pin);
            decryptWithKey(decdata);
        } catch (Exception ex) {
            DigiDocException.handleException(ex, DigiDocException.ERR_XMLENC_DECRYPT);
     ...
     * @throws DigiDocException for decryption errors
     */
    private void decryptWithKey(byte[] deckey) throws DigiDocException {
        ...
        try {
            m_transportKey = (SecretKey) new SecretKeySpec(deckey, DIGIDOC_ENCRYPTION_ALOGORITHM);
            // decrypt data
            Cipher cipher = getCipher(Cipher.DECRYPT_MODE, m_transportKey, ivdata);
            if (m_logger.isDebugEnabled()) m_logger.debug("Decrypting: " + m_data.length + " bytes");
            ...
            if (bPadOk) m_data = removePadding(m_data, nPadLen);
            // try second padding
            ...
            if (m_nDataStatus == DENC_DATA_STATUS_UNENCRYPTED_AND_COMPRESSED) decompress();
        } catch (Exception ex) {
            DigiDocException.handleException(ex, DigiDocException.ERR_XMLENC_DECRYPT);
        }
    }
    ...
    private boolean checkPadding(byte[] data, int nPadLen) {
        boolean bPadOk = true;
        if (m_logger.isDebugEnabled()) m_logger.debug("Checking padding: " + nPadLen + " bytes");
        ...
        for (int i = data.length - nPadLen; nPadLen > 0 && i < data.length - 1; i++) {
            if (data[i] != 0 && nPadLen != 16) {
                if (m_logger.isDebugEnabled()) m_logger.debug("Data at: " + i + " = " + data[i] + " cancel padding");
                bPadOk = false;
                break;
            }
        }
    ...
    private byte[] removePadding(byte[] data, int nPadLen) {
        if (m_logger.isDebugEnabled()) m_logger.debug("Removing padding: " + nPadLen + " bytes");
    ...
    /**
     * Encrypts all input data coming from this stream, compresses it if necessary converts to base64 and writes as an
     * XML encrypted
     * document to output stream. Use this method to encrypt big amunts of data. Please note that you must first
     * inittialize the
     * recipients (e.g. EncryptedKey objects) but not the data. Data comes from the input stream, thus data status
     * should be
     * UNINITIALIZED when you call this method.
     * 
     * @param in stream with data to encrypt
     * @param out output stream for encrypted data
     * @param nCompressOption compression option: allways, never. Best-effort is not supported here because we don't
     *            know it before we
    ...
    public void encryptStream(InputStream in, OutputStream out, int nCompressOption) throws DigiDocException {
        ...
        try {
            ...
            do {
                if (isReadFromFile) {
                    ...
                    if (nCompressOption == DENC_COMPRESS_ALLWAYS) {// compress
                        ...
                        if (m_logger.isDebugEnabled()) {
                            m_logger.debug("EncryptStream - input: " + nLen1 + " compressed: " + nLen2 + " needin: "
                ...
                if (encdata != null) {
                    ...
                    if (m_logger.isDebugEnabled()) {
                        m_logger.debug("EncryptStream - input: " + encdata.length + " used: " + nUsed + " copy: "
                                        + nB64left + " pos: " + nUsed);
                    }
                    if (nB64left > 0) {
                        ...
                        if (m_logger.isDebugEnabled()) {
                            m_logger.debug("EncryptStream - input: " + encdata.length + " used: " + nUsed + " base64: "
                                            + ((nUsed / 3) * 4) + " left: " + nB64left);
                        }
                    }
                }
            ...
            out.flush();
            if (m_logger.isInfoEnabled()) {
                m_logger.info("EncryptStream total - input: " + nTotalInput + " compressed: " + nTotalCompressed
                                + " encrypted: " + nTotalEncrypted + " base64: " + nTotalBase64);
            }
        } catch (Exception ex) {
            DigiDocException.handleException(ex, DigiDocException.ERR_XMLENC_ENCRYPT);
        }
    ...
    public List<DigiDocException> validate() {
        ...
        if (m_encProperties != null) {
            List<DigiDocException> e = m_encProperties.validate();
            if (!e.isEmpty()) {
                errs.addAll(e);
            }
        }
        for (int i = 0; i < getNumKeys(); i++) {
            EncryptedKey ekey = getEncryptedKey(i);
            List<DigiDocException> e = ekey.validate();
            if (!e.isEmpty()) {
                errs.addAll(e);
            }
        }
