...
public class BdocManifestParser extends DefaultHandler {
    ...
    private InputStream removeDtd(InputStream is) {
        try {
            ...
            if (m_logger.isDebugEnabled()) m_logger.debug("Manifest orig:\n------\n" + s + "\n------\n");
            data = null;
            ...
            if (p1 > 0) {
                ...
                if (p2 > 0) {
                    ...
                    if (m_logger.isDebugEnabled()) m_logger.debug("Manifest no-dtd:\n------\n" + s2 + "\n------\n");
    ...
    public Manifest readManifest(InputStream is) throws DigiDocException {
        ...
        if (m_logger.isDebugEnabled()) m_logger.debug("Start reading manifest.xml");
        try {
    ...
    public void startElement(String namespaceURI, String lName, String qName, Attributes attrs)
        ...
        String tag = qName;
        int p1 = tag.indexOf(':');
        if (p1 > 0) tag = qName.substring(p1 + 1);
        // <manifest>
        ...
        if (tag.equals("file-entry")) {
            ...
            for (int i = 0; i < attrs.getLength(); i++) {
                ...
                if (m_logger.isDebugEnabled()) m_logger.debug("attr: " + key);
            ...
            try {
                if (sPath.equals("/")) { // signed doc entry
                    ...
                    if (m_logger.isDebugEnabled())
                        m_logger.debug("Sdoc: " + m_sdoc.getFormat() + " / " + m_sdoc.getVersion() + " / "
                                        + m_sdoc.getProfile());
                } else if (sPath.indexOf("signature") != -1) { // signature entry
                    if (m_logger.isDebugEnabled()) m_logger.debug("Find sig: " + sPath + " type: " + sType);
                    if (sType.startsWith(MIME_SIGNATURE_BDOC) && !sType.equals(MIME_SIGNATURE_BDOC_BES)
                                    && !sType.equals(MIME_SIGNATURE_BDOC_T) && !sType.equals(MIME_SIGNATURE_BDOC_CL)
                                    && !sType.equals(MIME_SIGNATURE_BDOC_TM) && !sType.equals(MIME_SIGNATURE_BDOC_TS)
                                    && !sType.equals(MIME_SIGNATURE_BDOC_TMA) && !sType.equals(MIME_SIGNATURE_BDOC_TSA)) {
                    ...
                    if (sigProfile == null) {
                        if (sType.equals(MIME_SIGNATURE_BDOC_BES)) {
                            ...
                            if (m_sdoc.getProfile() == null || m_sdoc.getProfile().equals(SignedDoc.BDOC_PROFILE_BES)) // weakest profile to be set only if
                        ...
                        if (sType.equals(MIME_SIGNATURE_BDOC_T)) {
                            ...
                            if (m_sdoc.getProfile() == null || m_sdoc.getProfile().equals(SignedDoc.BDOC_PROFILE_BES)
                    ...
                    if (m_logger.isDebugEnabled()) m_logger.debug("Find df: " + sPath);
                    ...
                    if (df != null) {
                        ...
                        df = new DataFile(sPath, DataFile.CONTENT_BINARY, sPath, sType, m_sdoc);
