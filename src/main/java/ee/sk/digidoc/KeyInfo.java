 ...
 * info allways contains only one subject certificate, e.g. no uplinks and the
 * smaller items like RSA public key modulus and export are not kept separately
 * but calculated online from the signers certificate. That means they are
...
public class KeyInfo implements Serializable {
    ...
    private DigiDocException validateSignersCertificate(X509Certificate cert) {
        ...
        if (cert == null)
            ex = new DigiDocException(DigiDocException.ERR_SIGNERS_CERT, "Signers certificate is required", null);
        return ex;
    }
